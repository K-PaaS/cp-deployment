...
########################################################################################################################
# 4. alwaysPullImage 정책 생성 및 배포 (Kyverno 리소스 예외) #
########################################################################################################################

cat <<EOF > cp-always-pull-images-policy.yaml
apiVersion: kyverno.io/v1
kind: ClusterPolicy
metadata:
  name: cp-always-pull-images-policy
  annotations:
...
spec:
  rules:
  - name: cp-always-pull-images-policy-rule
    match:
      any:
      - resources:
          kinds:
...
EOF
kubectl apply -f cp-always-pull-images-policy.yaml
...
rm cp-always-pull-images-policy.yaml
